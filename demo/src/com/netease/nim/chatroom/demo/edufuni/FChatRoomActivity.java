package com.netease.nim.chatroom.demo.edufuni;
 ...
 */
public class FChatRoomActivity extends TActivity implements VideoListener {
    private static final String TAG = FChatRoomActivity.class.getSimpleName();
     ...
     * 子页面
     */
    private FChatRoomMessageFragment messageFragment;
    private FChatRoomFragment fragment;
    ...
    public static void start(Context context, String roomId, boolean isCreate) {
        ...
        intent.setClass(context, FChatRoomActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fchat_room_activity);
        ...
        public void onEvent(ChatRoomStatusChangeData chatRoomStatusChangeData) {
            if (chatRoomStatusChangeData.status == StatusCode.CONNECTING) {
                ...
                if(NIMClient.getService(ChatRoomService.class).getEnterErrorCode(roomId) == ResponseCode.RES_CHATROOM_STATUS_EXCEPTION) {
                    // 聊天室连接状态异常
                    Toast.makeText(FChatRoomActivity.this, R.string.chatroom_status_exception, Toast.LENGTH_SHORT).show();
                    NIMClient.getService(ChatRoomService.class).exitChatRoom(roomId);
                    if(fragment != null) {
                        fragment.onKickOut();
                    }
                } else {
                    Toast.makeText(FChatRoomActivity.this, R.string.nim_status_unlogin, Toast.LENGTH_SHORT).show();
                    if (fragment != null) {
                ...
                Toast.makeText(FChatRoomActivity.this, R.string.net_broken, Toast.LENGTH_SHORT).show();
                if (fragment != null) {
                    fragment.onOnlineStatusChanged(false);
                }
            }
        ...
        public void onEvent(ChatRoomKickOutEvent chatRoomKickOutEvent) {
            if (chatRoomKickOutEvent.getReason() == ChatRoomKickOutEvent.ChatRoomKickOutReason.CHAT_ROOM_INVALID) {
                if (!roomInfo.getCreator().equals(DemoCache.getAccount())) {
                    Toast.makeText(FChatRoomActivity.this, R.string.meeting_closed, Toast.LENGTH_SHORT).show();
                }
            } else if (chatRoomKickOutEvent.getReason() == ChatRoomKickOutEvent.ChatRoomKickOutReason.KICK_OUT_BY_MANAGER) {
                Toast.makeText(FChatRoomActivity.this, R.string.kick_out_by_master, Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(FChatRoomActivity.this, "被踢出聊天室，reason:" + chatRoomKickOutEvent.getReason(), Toast.LENGTH_SHORT).show();
            }
        ...
        enterRequest.setCallback(new RequestCallback<EnterChatRoomResultData>() {
            ...
            public void onFailed(int code) {
                ...
                if (code == ResponseCode.RES_CHATROOM_BLACKLIST) {
                    Toast.makeText(FChatRoomActivity.this, "你已被拉入黑名单，不能再进入", Toast.LENGTH_SHORT).show();
                } else if (code == ResponseCode.RES_ENONEXIST){
                    Toast.makeText(FChatRoomActivity.this, "该聊天室不存在", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(FChatRoomActivity.this, "enter chat room failed, code=" + code, Toast.LENGTH_SHORT).show();
                }
                finish();
            }
            ...
            public void onException(Throwable exception) {
                onLoginDone();
                Toast.makeText(FChatRoomActivity.this, "enter chat room exception, e=" + exception.getMessage(), Toast.LENGTH_SHORT).show();
                finish();
            }
        });
    }
    ...
    private void initChatRoomFragment(final String roomName) {
        fragment = (FChatRoomFragment) getSupportFragmentManager().findFragmentById(R.id.chat_rooms_fragment);
        if (fragment != null) {
            // TODO: 2018/6/21 ww_
            fragment.addFChatRoomFragment();
        ...
        if (roomInfo.getCreator().equals(DemoCache.getAccount())) {
            RTSManager2.getInstance().createSession(roomInfo.getRoomId(), "test", new RTSCallback<Void>() {
                ...
                public void onSuccess(Void aVoid) {
                    Toast.makeText(FChatRoomActivity.this, "创建多人白板房间成功", Toast.LENGTH_SHORT).show();
                    joinRTSSession();
                }
                ...
                public void onFailed(int i) {
                    if (i == 417) {
                        Toast.makeText(FChatRoomActivity.this, "多人白板房间已经被预定", Toast.LENGTH_SHORT).show();
                        joinRTSSession();
                    }
    ...
    private void joinRTSSession() {
        ...
        RTSManager2.getInstance().joinSession(roomInfo.getRoomId(), isOpen, new RTSCallback<RTSData>() {
            ...
            public void onSuccess(RTSData rtsData) {
                ...
                Toast.makeText(FChatRoomActivity.this, "加入多人白板房间成功", Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onFailed(int i) {
                LogUtil.d(TAG, "join rts session failed, code:" + i);
                Toast.makeText(FChatRoomActivity.this, "join rts session failed, code:" + i, Toast.LENGTH_SHORT).show();
            }
    ...
    private RTSChannelStateObserver channelStateObserver = new RTSChannelStateObserver() {
        ...
        public void onConnectResult(String localSessionId, RTSTunnelType tunType, long channelId, int code, String recordFile) {
            Toast.makeText(FChatRoomActivity.this, "onConnectResult, tunType=" + tunType.toString() +
                    ", channelId=" + channelId +
            ...
            if (roomInfo.getCreator().equals(DemoCache.getAccount())) {
                ...
                Toast.makeText(FChatRoomActivity.this, "send sync request", Toast.LENGTH_SHORT).show();
        ...
        public void onChannelEstablished(String sessionId, RTSTunnelType tunType) {
            Toast.makeText(FChatRoomActivity.this, "onCallEstablished,tunType=" + tunType.toString(), Toast
                    .LENGTH_SHORT).show();
        }
        ...
        public void onDisconnectServer(String sessionId, RTSTunnelType tunType) {
            Toast.makeText(FChatRoomActivity.this, "onDisconnectServer, tunType=" + tunType.toString(), Toast
                    .LENGTH_SHORT).show();
            if (tunType == RTSTunnelType.DATA) {
                // 如果数据通道断了，那么关闭会话
                Toast.makeText(FChatRoomActivity.this, "TCP通道断开，自动结束会话", Toast.LENGTH_SHORT).show();
                RTSManager2.getInstance().leaveSession(sessionId, null);
        ...
        public void onError(String sessionId, RTSTunnelType tunType, int code) {
            Toast.makeText(FChatRoomActivity.this, "onError, tunType=" + tunType.toString() + ", error=" + code,
