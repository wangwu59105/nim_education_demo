...
import com.netease.nim.chatroom.demo.im.ui.listview.ListViewUtil;
...
import com.netease.nim.chatroom.demo.education.viewholder.ChatRoomMsgViewHolderFactory;
...
//        MessageAudioControl.getInstance(container.activity).stopAudio(); // 界面返回，停止语音播放
    ...
    private class MessageLoader implements AutoRefreshListView.OnRefreshListener {
        ...
        private void onMessageLoaded(List<ChatRoomMessage> messages) {
            ...
            for (IMMessage message : messages) {
                if (message.getAttachment() != null && message.getAttachment() instanceof PermissionAttachment) {
                    // 在线教育内的控制消息过滤
                    continue;
                }
                result.add(message);
